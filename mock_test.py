"""mock_test.py — timed SAT-style mock exam"""
import random
import time
import streamlit as st
from helpers import section_header, badge_html, update_score_after_answer
from ai_engine import QUESTION_BANK, explain_question


def render():
    section_header("Mock Test",
                   "Digital-SAT-style timed simulation with scoring and review.")
...
This mock runs a **condensed simulation** with curated questions across both
sections, plus a live timer, instant scoring, and a per-question review.
""")
...
def _render_test():
    ...
    cols[0].markdown(f"**Question {idx+1} / {total}**")
    cols[1].markdown(
        f"<div style='text-align:center;font-family:var(--mono);'>"
        f"<span style='color:var(--text-mute);font-size:0.75rem;'>time remaining</span> "
        f"<span style='color:var(--text);font-weight:600;font-size:1rem;margin-left:0.5rem;'>"
        f"{mins:02d}:{secs:02d}</span></div>",
        unsafe_allow_html=True
    )
    cols[2].markdown(
        f"<div style='text-align:right;font-family:var(--mono);font-size:0.85rem;'>"
        f"<span style='color:var(--text-mute);'>answered</span> "
        f"<span style='color:var(--text);font-weight:600;'>{answered_count}/{total}</span></div>",
        unsafe_allow_html=True
    )
...
def _render_results():
    ...
    section_header("Test Complete", f"Time used: {mins}m {secs}s")
    ...
    c4.metric("vs current", f"{delta:+d}")
    ...
    for i, (q, a) in enumerate(zip(questions, answers)):
        ...
        marker = "✓" if is_correct else ("–" if a is None else "✗")
        with st.expander(f"[{marker}] Q{i+1}: {q['question'][:80]}{'…' if len(q['question'])>80 else ''}"):
            ...
            if st.button("Detailed walkthrough", key=f"review_ai_{i}"):
    ...
    c1, c2 = st.columns(2)
    with c1:
        if st.button("Take another", use_container_width=True, type="primary"):
